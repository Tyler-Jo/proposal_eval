...
* [x] 제안서 평가에서 PDF 텍스트·OCR 결과를 파일 경로/크기/수정시각 기준으로 sidecar 메모리에 캐시한다. 동일 원본 재평가는 OCR을 다시 수행하지 않는다.
* [x] 규칙 기반 판정과 로컬 Gemma 코멘트를 분리했다. 평가 완료는 규칙 판정·근거 페이지 생성 시점으로 앞당기고, AI 코멘트는 사용자가 요청할 때 별도 작업으로 생성한다.
...
* OCR 결과와 규칙 평가 결과는 온디바이스 SQLite(`proposal_evaluation.sqlite3`)에 저장한다. 원본 PDF는 복사하지 않고 경로와 SHA-256으로 식별하며, 같은 원본·설정은 sidecar 재시작 뒤에도 OCR 결과를 재사용한다. 기본 저장 위치는 Windows `%LOCALAPPDATA%\\AVISCheck`, 그 외 OS는 사용자 상태 데이터 폴더이며 `PROPOSAL_EVALUATION_DATA_DIR`로 변경할 수 있다. (저장 경로 식별자는 기존 캐시 호환성을 위해 유지한다.)
